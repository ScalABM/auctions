...
public class Sandbox {
    ...
    public static void main(String[] args) {
        ...
        JSealedBidAuction<TestStock> fbsba = new JSealedBidAuction<TestStock>(order3, askQuotePricing, 1L);
        JSealedBidAuction<TestStock> fpsba2 = fbsba.insert(order8).get();
        JSealedBidAuction<TestStock> fpsba3 = fpsba2.insert(order9).get();
        JClearResult<JSealedBidAuction<TestStock>> results = fpsba3.clear();
        System.out.println(results.getFills().get());  // TODO: is Stream the best collection to use here?

        JOpenBidDoubleAuction.DiscriminatoryPricingImpl<TestStock> da = new JOpenBidDoubleAuction().withDiscriminatoryPricing(midPointPricing, 1L);
        JOpenBidDoubleAuction.DiscriminatoryPricingImpl<TestStock> da2 = da.insert(order3).get();
        JOpenBidDoubleAuction.DiscriminatoryPricingImpl<TestStock> da3 = da2.insert(order4).get();
        JOpenBidDoubleAuction.DiscriminatoryPricingImpl<TestStock> da4 = da3.insert(order8).get();

        System.out.println(da4.receive(new AskPriceQuoteRequest<>()));
        System.out.println(da4.receive(new BidPriceQuoteRequest<>()));

        JOpenBidDoubleAuction.DiscriminatoryPricingImpl<TestStock> da5 = da4.insert(order9).get();
        JClearResult<JOpenBidDoubleAuction.DiscriminatoryPricingImpl<TestStock>> results3 = da5.clear();
