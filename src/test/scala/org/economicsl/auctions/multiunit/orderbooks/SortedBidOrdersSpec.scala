...
class SortedBidOrdersSpec extends FlatSpec with Matchers {
  ...
  "A SortedBidOrderBook" should "update an existing order" in {
    ...
    val order = multiunit.LimitBidOrder(issuer, Price(10), Quantity(100), google)
  ...
  "A SortedBidOrderBook" should "split itself into two pieces" in {
    ...
    val order1 = multiunit.LimitBidOrder(issuer1, Price(10), Quantity(10), google)

    val issuer2 = UUID.randomUUID()
    val order2 = multiunit.LimitBidOrder(issuer2, Price(5), Quantity(15), google)
